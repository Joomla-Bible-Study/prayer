...
$img = JHTML::_('image', JURI::base() . 'media/system/images/weblink.png', htmlentities(JText::_('CWMPRAYERFEEDS')), 'style="border:0;"');
...
{
	echo '<div class="componentheading"><h2>' . htmlentities($this->title . ' - ' . JText::_('CWMPRAYERLINKSLIST')) . '</h2></div>';
}
...
$prayer->writePCImage() . '</div><div>';


echo $prayer->writePCHeader($this->intro) . '</div>';


echo '<fieldset class="pcmod"><legend>' . htmlentities(JText::_('CWMPRAYERLINKSLIST')) . '</legend>';
...
else
{
	...
	echo '<div><strong><div class="content"><br />' . htmlentities(JText::_('CWMPRAYERNOLINKS')) . '</div></strong>
