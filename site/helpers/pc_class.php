...
class prayer
{
	...
	function intializePCRights()
	{
		...
		if (JAccess::check($user->get('id'), 'prayer.view', 'com_prayer')) $pc_rights->set('pc.view', true);
		if (JAccess::check($user->get('id'), 'prayer.post', 'com_prayer')) $pc_rights->set('pc.post', true);
		if (JAccess::check($user->get('id'), 'prayer.publish', 'com_prayer')) $pc_rights->set('pc.publish', true);
		if (JAccess::check($user->get('id'), 'prayer.subscribe', 'com_prayer')) $pc_rights->set('pc.subscribe', true);
		if (JAccess::check($user->get('id'), 'prayer.devotional', 'com_prayer')) $pc_rights->set('pc.view_devotional', true);
		if (JAccess::check($user->get('id'), 'prayer.links', 'com_prayer')) $pc_rights->set('pc.view_links', true);
	...
	function PCgetButtons($showrequest, $editonly = false)
	{
		...
		if (!$editonly)
		{
			...
			$pdf_link = 'index.php?option=com_prayer&amp;task=pdf&amp;listtype=0&amp;id=' . $showrequest->id . '&amp;title=' . htmlentities(JText::_('PCTITLE')) . '&amp;format=pdf';
			...
			$print_link = "index.php?option=com_prayer&amp;task=view_request&amp;id=" . $showrequest->id . "&amp;pop=1&amp;prt=1&amp;tmpl=component&amp;Itemid=" . $itemid;
			$status     = 'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no';
	...
	function PCgetProfileBox($requestarr, $showavatar = true)
	{
		...
		if ($pcConfig['config_community'] == 1)
		{
			...
			$reqcount ? $results .= "<br />&nbsp;&nbsp;" . ucfirst(strtolower(htmlentities(JText::_('PCPRAYERREQUESTS')))) . "&nbsp;<a href=\"" . JRoute::_('index.php?option=com_prayer&task=view&searchrequester=' . $requester . '&searchrequesterid=' . $reqid) . "\">" . $reqcount . "</a>" : $results .= "<br />";
	...
	function PCgetComments($showrequest, $showcomments = false)
	{
		...
		if ($pcConfig['config_comments'] == 1)
		{
			...
			if (!$showcomments)
			{
				...
				{
					$return = '<a href="' . JRoute::_("index.php?option=com_prayer&task=view_request&id=" . $showrequest->id . "&pop=0&Itemid=" . $itemid) . '#comments" />' . JText::_('PCCOMMENTS') . '&nbsp;(' . $commentsCount . ')</a>';
				}
			}
			else
			{
	...
	{
		$return  = '<div class="pcsearch" id="pcsearchbox"><form action="' . JRoute::_('index.php?option=com_prayer&task=view') . '" name="searchPC" method="post">';
	...
	function PCgetSizeRequest($showrequest)
	{
		...
		if (($pcConfig['config_req_length'] > 0) && (strlen($showrequest->text) > $pcConfig['config_req_length']))
		{
			...
			$return            = '<div class="reqcontent">"' . $this->PCkeephtml(JText::_($this->PCstripslashes($showrequest->text))) . '"<small>&nbsp;&nbsp;<a href="index.php?option=com_prayer&task=view_request&id=' . $showrequest->id . '&Itemid=' . $itemid . '" /><i><span style="white-space:nowrap;">' . htmlentities(JText::_('PCREADMORE')) . '</span></i></a></small><br /><br />';
		}
		else
		{
...
//    $res = htmlentities($string);
	...
	function PCCaptchaValidate($usercode, $page)
	{
		...
		if ($session->get('pc_max_attempts') == $maxattempts)
		{
			...
			{
				$returnurl = JRoute::_('index.php?option=com_prayer&task=' . $page . '&Itemid=' . $itemid . '&return_msg=' . JText::_('PCCAPTCHAMAXATTEMPTS'));
				$this->PCRedirect($returnurl);
...
//    if($_POST['formtime'] < time()-3600) $spam = 1;
	...
	function buildPCMenu($mod = false, $modparams = null)
	{
		...
		if ($show_view && $this->pc_rights->get('pc.view'))
		{
			...
			echo '<a class="' . $menuclass . $moduleclass_sfx . '" title="' . htmlentities(JText::_('PCVIEWLIST')) . '" href="' . JRoute::_("index.php?option=com_prayer&task=view&Itemid=$itemid") . '">
		...
		{
			echo $menu_style == 0 ? '<div align="left">' : '<li class="pc-modmenu">';
			echo '<a class="' . $menuclass . $moduleclass_sfx . '" title="' . htmlentities(JText::_('PCSUBMITREQUEST')) . '" href="' . JRoute::_("index.php?option=com_prayer&task=newreq&Itemid=$itemid") . '">
		...
		{
			echo $menu_style == 0 ? '<div align="left">' : '<li class="pc-modmenu">';
			echo '<a class="' . $menuclass . $moduleclass_sfx . '" title="' . htmlentities(JText::_('PCSUBSCRIBE')) . '" href="' . JRoute::_("index.php?option=com_prayer&task=subscribe&Itemid=$itemid") . '">
		...
		{
			echo $menu_style == 0 ? '<div align="left">' : '<li class="pc-modmenu">';
			echo '<a class="' . $menuclass . $moduleclass_sfx . '" title="' . htmlentities(JText::_('PCLINKSLIST')) . '" href="' . JRoute::_("index.php?option=com_prayer&task=view_links&Itemid=$itemid") . '">
		...
		{
			echo $menu_style == 0 ? '<div align="left">' : '<li class="pc-modmenu">';
			echo '<a class="' . $menuclass . $moduleclass_sfx . '" title="' . htmlentities(JText::_('PCDEVOTIONALS')) . '" href="' . JRoute::_("index.php?option=com_prayer&task=view_devotion&Itemid=$itemid") . '">
		...
		{
			echo $menu_style == 0 ? '<div align="left">' : '<li class="pc-modmenu">';
			echo '<a class="' . $menuclass . $moduleclass_sfx . '" title="' . htmlentities(JText::_('PCMODERATORS')) . '" href="' . JRoute::_("index.php?option=com_prayer&task=moderate&Itemid=$itemid") . '">
			...
			if (JFolder::exists($abpath_folder))
			{
                ...
                <div class="mosimage" align="center" style="float:right;padding:0">
                    <img src="<?php echo $image; ?>" name="pcslide" border="<?php echo $border; ?>"
                         style="filter:blendTrans(duration=<?php echo $pcConfig['config_slideshow_duration']; ?>)"
