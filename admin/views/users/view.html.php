 ...
 * @since    4.0
 */
class CWMPrayerViewUsers extends JViewLegacy
{
	protected $items;
	 ...
	 * Display
	 *
	 * @param   null  $tpl  Template option
	 *
	 * @return bool|mixed
	 *
	 * @since 4.0
	...
	public function display($tpl = null)
	{
		...
		if ($search)
		{
			$searches[] = '(a.name LIKE ' . $db->q('%' . $db->escape($search, true) . '%', false);
			$searches[] = 'a.username LIKE ' . $db->q('%' . $db->escape($search, true) . '%', false);
			$searches[] = 'a.email LIKE ' . $db->q('%' . $db->escape($search, true) . '%', false) . ')';
		}
