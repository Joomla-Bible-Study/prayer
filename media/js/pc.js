...
function PCgetImage(livesite, action) {
	...
	document.images[sec_image].src = livesite + "components/com_cwmprayer/assets/captcha/prayer.captcha.inc.php?action=" + action + "&new=" + escape(new Date().getTime());
...
function checkPCCapCode(theForm, usercode, livesite, action) {
	...
	var url = livesite + 'index.php?option=com_cwmprayer&tmpl=component&task=PCCapValid&';
	...
	if (receivePCReq.readyState == 4 || receivePCReq.readyState == 0) {
		...
		receivePCReq.onreadystatechange = function () {
			updatePCCapPage(theForm, livesite, action);
		}
...
function checkReCapCode(theForm, usercode, livesite) {
	...
	var url = livesite + 'index.php?option=com_cwmprayer&tmpl=component&format=raw&task=PCCapValid&';
